...
- NFS/disk systemd units and `/etc` restore run through a root-owned helper at `/usr/lib/omaclone/privileged.py` (owner, mode, and digest checked). Inherited helper payloads are ignored. Disk format requires `/dev/disk/by-id`, derives the sysfs device graph itself, and refuses system/swap/mounted relations. Uninstall removes only unit files whose content hash matches this installation.
